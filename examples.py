...
def example_05_annotations():
	...
	myTokens = tx.TokenizerView()
	myTokens.add( tx.Token("Barack") )
	myTokens.add( tx.Token("Obama") )
	myTokens.add( tx.Token("served") )
	myTokens.add( tx.Token("as") )
	myTokens.add( tx.Token("the") )
	print ( "myDoc.JSON = " + myDoc.getJSON() )
	print("---")
